...
try {
    ...
    if (!$profil) die("Data profil tidak ditemukan.");
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        function prosesUpload($file_input, $prefix, $foto_lama)
        {
            if (isset($_FILES[$file_input]) && $_FILES[$file_input]['error'] !== 4) {
                ...
                if (!in_array($ekstensi_file, $ekstensi_valid)) throw new Exception("Ekstensi foto tidak valid. Gunakan JPG/PNG.");
                if ($ukuran_file > 3000000) throw new Exception("Ukuran foto maksimal 3MB.");
        ...
        $sqlUpdate = "UPDATE tabel_alumni_profil SET nama_lengkap = :nama, tahun_masuk = :thn_masuk, tahun_lulus = :thn_lulus, angkatan = :angkatan, usia = :usia, jalur_masuk = :jalur, jabatan_sekarang = :jabatan, perusahaan_sekarang = :perusahaan, domisili = :domisili, linkedin_url = :linkedin, ringkasan_profesional = :ringkasan, foto_profil = :foto_profil, foto_sampul = :foto_sampul WHERE user_id = :user_id";

        $stmtUpdate = $koneksi->prepare($sqlUpdate);
        $stmtUpdate->execute([':nama' => $nama_lengkap, ':thn_masuk' => $tahun_masuk, ':thn_lulus' => $tahun_lulus, ':angkatan' => $angkatan, ':usia' => $usia, ':jalur' => $jalur_masuk, ':jabatan' => $jabatan_sekarang, ':perusahaan' => $perusahaan_sekarang, ':domisili' => $domisili, ':linkedin' => $linkedin_url, ':ringkasan' => $ringkasan, ':foto_profil' => $foto_profil_baru, ':foto_sampul' => $foto_sampul_baru, ':user_id' => $_SESSION['user_id']]);
...
{
    if (empty($foto) || strpos($foto, 'default-') === 0) return $jenis == 'avatar' ? 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?ixlib=rb-4.0.3&auto=format&fit=crop&w=400&q=80' : 'https://images.unsplash.com/photo-1497366216548-37526070297c?ixlib=rb-4.0.3&auto=format&fit=crop&w=1200&q=80';
    ...
    <aside id="sidebar-dashboard" class="hidden absolute inset-y-0 left-0 z-50 md:relative md:flex flex-col w-64 bg-udinus-navy text-white h-full shadow-2xl transition-transform duration-300">
        ...
        <div class="overflow-y-auto overflow-x-hidden flex-grow py-6">
            <ul class="flex flex-col py-4 space-y-2 px-4">
                <li><a href="dashboard.php" class="relative flex flex-row items-center h-11 hover:bg-white/10 text-gray-300 hover:text-white border-l-4 border-transparent hover:border-white rounded-r-lg px-3 transition duration-300">
                ...
                <li><a href="edit-profil.php" class="relative flex flex-row items-center h-11 bg-white/5 text-udinus-gold border-l-4 border-udinus-gold rounded-r-lg px-3 transition duration-300">
                        <svg class="w-5 h-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="px-8 pb-8 relative">
                        <div class="w-32 h-32 rounded-full border-4 border-white shadow-lg overflow-hidden relative group -mt-16 bg-gray-200 z-10">
                            <img id="preview-avatar" src="<?php echo htmlspecialchars($fotoProfilAktif); ?>" class="w-full h-full object-cover">
                            <div class="absolute inset-0 bg-black/40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                        ...
                        <p class="text-xs text-gray-400 mt-2 font-semibold">*Format didukung: JPG, PNG. Maksimal 3MB.</p>
                    </div>
                </div>
