...
try {
    // Standardisasi Variabel: $profil
    $queryProfil = $koneksi->prepare("SELECT u.email, u.phone, p.* FROM tabel_users u JOIN tabel_alumni_profil p ON u.id = p.user_id WHERE u.id = :user_id");
    $queryProfil->execute([':user_id' => $_SESSION['user_id']]);
    $profil = $queryProfil->fetch();

    if (!$profil) die("Data profil tidak ditemukan. Silakan hubungi Administrator.");
...
<body class="bg-gray-50 font-sans antialiased flex h-screen overflow-hidden text-gray-800">
    ...
    <aside id="sidebar-dashboard" class="hidden absolute inset-y-0 left-0 z-50 md:relative md:flex flex-col w-64 bg-udinus-navy text-white h-full shadow-2xl transition-transform duration-300">
        <div class="flex items-center justify-center h-20 border-b border-white/10 gap-3 px-6">
            <div class="w-10 h-10 bg-white rounded flex items-center justify-center p-1 overflow-hidden"><img src="assets/images/logo-udinus.png" alt="Logo SI" class="w-full h-full object-contain"></div>
        ...
        <div class="overflow-y-auto overflow-x-hidden flex-grow py-6">
            <ul class="flex flex-col py-4 space-y-2 px-4">
                <li><a href="dashboard.php" class="relative flex flex-row items-center h-11 bg-white/5 text-udinus-gold border-l-4 border-udinus-gold rounded-r-lg px-3 transition duration-300">
                        <svg class="w-5 h-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"></path>
                        </svg>
                        <span class="ml-3 text-sm font-bold tracking-wide truncate">Beranda Alumni</span>
                    </a></li>
                <li><a href="edit-profil.php" class="relative flex flex-row items-center h-11 hover:bg-white/10 text-gray-300 hover:text-white border-l-4 border-transparent hover:border-white rounded-r-lg px-3 transition duration-300">
                        <svg class="w-5 h-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                        </svg>
                        <span class="ml-3 text-sm font-medium tracking-wide truncate">Profil & Karir Saya</span>
                    </a></li>
                <li><a href="tracer-study.php" class="relative flex flex-row items-center h-11 hover:bg-white/10 text-gray-300 hover:text-white border-l-4 border-transparent hover:border-white rounded-r-lg px-3 transition duration-300">
                        <svg class="w-5 h-5 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <header class="h-20 bg-white shadow-sm flex items-center justify-between px-6 md:px-8 z-30 relative">
            ...
            <div class="flex items-center text-lg font-bold text-udinus-navy">Dashboard Interaktif</div>

            <div class="relative">
                <button id="btn-profil" class="flex items-center gap-4 focus:outline-none hover:opacity-80 transition cursor-pointer">
                ...
                <div id="dropdown-profil" class="hidden absolute right-0 mt-3 w-48 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-50">
                    <button onclick="bukaModalAkun()" class="w-full text-left px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:text-udinus-navy transition flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        </svg>Manage Akun</button>
                    <div class="border-t border-gray-100 my-1"></div>
                    <button class="w-full text-left px-4 py-2 text-sm font-semibold text-red-600 hover:bg-red-50 transition flex items-center gap-2"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-50 p-6 md:p-8">
            ...
            <?php if (!$sudah_tracer): ?>
                <div class="bg-udinus-navy rounded-2xl shadow-sm p-6 text-white mb-10 flex flex-col md:flex-row justify-between items-center gap-6 relative overflow-hidden">
                    ...
                    <a href="tracer-study.php" class="bg-udinus-gold hover:bg-yellow-500 text-white font-bold py-3 px-8 rounded-lg shadow-md transition duration-300 whitespace-nowrap z-10">Mulai Pengisian</a>
                </div>
            <?php else: ?>
                <div class="bg-green-600 rounded-2xl shadow-sm p-6 text-white mb-10 flex items-center gap-4 relative overflow-hidden">
                    ...
                    <div class="p-3 bg-white/20 rounded-full z-10"><svg class="w-8 h-8 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="mb-8">
                ...
                <p class="text-gray-500 mt-1">Temukan dan perluas relasi dengan alumni lain. Profil ditampilkan secara (read-only).</p>
            </div>

            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-10">
                <?php if (empty($jejaringAlumni)): ?>
                    <div class="col-span-full text-center py-10 text-gray-400 font-semibold">Belum ada alumni lain yang terdaftar dalam jejaring ini.</div>
                <?php else: ?>
                    <?php foreach ($jejaringAlumni as $alumniLain): ?>
                        <a href="detail-alumni.php?id=<?php echo $alumniLain['id']; ?>" class="bg-white rounded-xl shadow-sm hover:shadow-xl transition duration-300 border border-gray-100 p-6 flex flex-col items-center text-center group block">
                            ...
                            <p class="text-sm font-semibold text-gray-700 bg-gray-50 py-1 px-3 rounded-full mt-auto w-full truncate"><?php echo htmlspecialchars($alumniLain['jabatan_sekarang'] ?? 'Alumni'); ?></p>
                        </a>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
    ...
    <div id="modal-akun" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden transform scale-100">
            <div class="bg-udinus-navy p-5 flex justify-between items-center">
                ...
                <button onclick="tutupModalAkun()" class="text-gray-300 hover:text-white focus:outline-none"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                    </svg></button>
            </div>
            <form onsubmit="event.preventDefault(); tutupModalAkun();" class="p-6 space-y-5">
                <div><label class="block text-sm font-semibold text-gray-700 mb-2">Email Terdaftar</label><input type="email" value="<?php echo htmlspecialchars($profil['email']); ?>" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 bg-gray-50 text-gray-500 cursor-not-allowed" readonly></div>
                <div><label class="block text-sm font-semibold text-gray-700 mb-2">Nomor Telepon Aktif</label><input type="text" value="<?php echo htmlspecialchars($profil['phone']); ?>" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy text-gray-800"></div>
                <div><label class="block text-sm font-semibold text-gray-700 mb-2">Kata Sandi Baru (Opsional)</label><input type="password" placeholder="Kosongkan jika tidak ingin mengubah" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy text-gray-800 placeholder-gray-400"></div>
                <div class="pt-4 flex justify-end gap-3"><button type="button" onclick="tutupModalAkun()" class="px-5 py-2.5 rounded-lg text-gray-600 font-semibold hover:bg-gray-100 transition">Batal</button><button type="submit" class="px-5 py-2.5 rounded-lg bg-udinus-navy hover:bg-blue-900 text-white font-bold shadow-md transition">Simpan Kredensial</button></div>
            </form>
        </div>
    </div>
