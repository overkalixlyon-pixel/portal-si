...
<body class="bg-gray-50 text-gray-800 font-sans antialiased selection:bg-udinus-gold selection:text-white flex flex-col min-h-screen">
    ...
    <section class="relative bg-udinus-navy pt-24 pb-20 md:pt-32 md:pb-28 overflow-hidden border-b-[6px] border-udinus-gold">
        ...
        <div class="container mx-auto px-6 text-center relative z-10">
            ...
            <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white mb-4 tracking-tight">
                Jejak Gemilang Lulusan
            </h1>
            <p class="text-blue-100 text-lg md:text-xl max-w-2xl mx-auto font-medium leading-relaxed">
                Kumpulan rekam jejak dedikasi, inovasi, dan torehan prestasi akademik maupun non-akademik mahasiswa dan jejaring alumni Sistem Informasi UDINUS.
            </p>
        </div>
    </section>
    ...
    <section class="py-16 bg-gray-50 flex-grow">
        <div class="container mx-auto px-6">
            ...
            <div class="flex flex-wrap justify-center gap-3 md:gap-4 mb-16">
                <button onclick="filterPrestasi('semua')" id="btn-semua" class="filter-btn bg-udinus-navy text-white px-7 py-3 rounded-xl font-bold transition duration-300 shadow-lg shadow-blue-900/10 text-sm tracking-wide">
                    Semua Kategori
                </button>
                <button onclick="filterPrestasi('himpunan')" id="btn-himpunan" class="filter-btn bg-white text-gray-600 border border-gray-200 hover:border-udinus-navy hover:text-udinus-navy px-7 py-3 rounded-xl font-bold transition duration-300 text-sm tracking-wide shadow-sm">
                    Internal Mahasiswa / Ormawa
                </button>
                <button onclick="filterPrestasi('alumni')" id="btn-alumni" class="filter-btn bg-white text-gray-600 border border-gray-200 hover:border-udinus-navy hover:text-udinus-navy px-7 py-3 rounded-xl font-bold transition duration-300 text-sm tracking-wide shadow-sm">
                    Eksternal / Portofolio Alumni
                </button>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto" id="grid-prestasi">
                ...
                <?php else: ?>
                    ...
                    <?php foreach ($daftarPrestasi as $prestasi): ?>
                        <div class="kartu-prestasi bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl border border-gray-100 group flex flex-col animate-[fadeIn_0.5s_ease-in-out]" data-kategori="<?php echo htmlspecialchars($prestasi['kategori']); ?>">

                            <div class="h-56 overflow-hidden bg-gray-200 relative">
                                <span class="absolute top-4 left-4 bg-udinus-navy/90 backdrop-blur-md text-udinus-gold text-[10px] font-extrabold px-3 py-1.5 rounded-lg z-10 shadow-md uppercase tracking-wider border border-white/10">
                                    <?php echo htmlspecialchars($prestasi['kategori'] === 'himpunan' ? 'Mahasiswa/HMP' : 'Alumni'); ?>
                                </span>
                            ...
                            <div class="p-7 flex flex-col flex-grow relative">
                                <div class="absolute -top-4 right-6 bg-udinus-gold text-white text-xs font-extrabold px-4 py-1 rounded-full shadow-md border border-white">
                                    Tahun <?php echo htmlspecialchars($prestasi['tahun']); ?>
                                </div>
                                <h3 class="text-xl font-extrabold text-gray-900 mb-3 leading-snug group-hover:text-udinus-navy transition-colors duration-300"><?php echo htmlspecialchars($prestasi['judul_prestasi']); ?></h3>
                                <p class="text-gray-500 text-sm leading-relaxed mb-6 flex-grow text-justify font-medium"><?php echo htmlspecialchars($prestasi['deskripsi']); ?></p>

                                <div class="w-full border-t border-gray-100 pt-4 mt-auto">
                                    <a href="#" class="text-udinus-gold font-bold text-xs hover:text-yellow-600 transition duration-300 inline-flex items-center gap-1.5 uppercase tracking-wider">
                                        Eksplorasi Detail Berita <span>&rarr;</span>
                                    </a>
                                </div>
                            </div>
    ...
    <script>
        function filterPrestasi(kategori) {
            ...
            // Aktifkan visual tombol yang sedang dipilih
            ...
            // Manipulasi DOM Grid Sesuai Atribut Kategori
            kartu.forEach(k => {
                if (kategori === 'semua') {
