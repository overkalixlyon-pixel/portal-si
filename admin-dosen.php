...
if (isset($_GET['sukses'])) {
    if ($_GET['sukses'] == 'tambah') $pesan_sukses = "Data staf pengajar baru berhasil ditambahkan!";
    if ($_GET['sukses'] == 'edit') $pesan_sukses = "Pembaruan data staf pengajar berhasil disimpan!";
    if ($_GET['sukses'] == 'hapus') $pesan_sukses = "Data staf pengajar beserta fotonya berhasil dihapus!";
}
...
try {
    ...
    if (isset($_GET['hapus'])) {
        ...
        if ($fotoLama) {
            ...
            // Hapus file fisik jika ada (mencegah penumpukan file server)
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        // Teks sambutan dikhususkan bagi Kaprodi, bisa dikosongkan untuk dosen biasa
        ...
        if ($aksi === 'tambah') {
            if (!$upload_sukses) {
                throw new Exception("Foto resmi dosen wajib diunggah untuk penambahan data baru!");
            }
            
            $sqlInsert = "INSERT INTO tabel_dosen (nama_dosen, jabatan_akademik, kepakaran, foto_dosen, sambutan_teks, urutan_tampil) 
                          VALUES (:nama, :jabatan, :kepakaran, :foto, :sambutan, :urutan)";
            ...
            if ($upload_sukses) {
                ...
                if ($fotoLama && file_exists('assets/images/' . $fotoLama['foto_dosen'])) {
    ...
    // Mengurutkan berdasarkan urutan_tampil (Kaprodi di atas) lalu nama dosen
    // =========================================================================
...
<body class="bg-gray-50 font-sans antialiased flex h-screen overflow-hidden text-gray-800">
    ...
    <aside id="sidebar-admin" class="hidden absolute inset-y-0 left-0 z-50 md:relative md:flex flex-col w-64 bg-gray-900 text-white h-full shadow-2xl transition-transform duration-300">
        ...
        <div class="overflow-y-auto overflow-x-hidden flex-grow py-6">
            <ul class="flex flex-col py-4 space-y-2 px-4">
                ...
                <li>
                    <a href="admin-prestasi.php" class="flex flex-row items-center h-11 text-gray-400 hover:text-white hover:bg-gray-800 rounded-lg px-4 transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z"></path></svg>
                        <span class="ml-3 text-sm">Kelola Prestasi</span>
                    </a>
                </li>
                <li>
                    <a href="admin-dosen.php" class="flex flex-row items-center h-11 text-udinus-gold bg-white/10 rounded-lg px-4 transition">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
                        <span class="ml-3 text-sm font-semibold">Kelola Dosen & Staf</span>
                    </a>
                </li>
                <li>
    ...
    <div class="flex-1 flex flex-col h-full relative overflow-hidden">
        ...
        <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-100 p-6 md:p-8">
            ...
            <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                <h1 class="text-xl font-bold text-gray-800">Direktori Dosen Aktif</h1>
                        ...
                        <tbody>
                            ...
                            <?php else: ?>
                                ...
                                <tr class="border-b hover:bg-gray-50 transition">
                                    <td class="px-6 py-4">
                                        <div class="w-14 h-14 rounded-full overflow-hidden shadow-sm border border-gray-200 bg-gray-200">
                                            <img src="assets/images/<?php echo htmlspecialchars($dosen['foto_dosen']); ?>" alt="Foto" class="w-full h-full object-cover">
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <h3 class="font-bold text-gray-900 text-base mb-1 leading-tight"><?php echo htmlspecialchars($dosen['nama_dosen']); ?></h3>
                                        <p class="text-xs font-semibold text-gray-500"><?php echo htmlspecialchars($dosen['jabatan_akademik']); ?></p>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 text-center">
                                        <?php if ($dosen['urutan_tampil'] == 1): ?>
                                            <span class="bg-blue-100 text-udinus-navy px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Pimpinan</span>
                                        <?php else: ?>
                                            <span class="bg-gray-100 text-gray-500 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Reguler</span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center justify-center gap-2">
                                            <button onclick="editData(<?php echo htmlspecialchars(json_encode($dosen)); ?>)" class="p-2 text-udinus-gold hover:bg-yellow-50 rounded transition" title="Edit">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </button>
                                            <a href="javascript:void(0);" onclick="konfirmasiHapus(<?php echo $dosen['id']; ?>)" class="p-2 text-red-500 hover:bg-red-50 rounded transition" title="Hapus">
    ...
    <div id="modal-form" class="fixed inset-0 z-50 hidden items-center justify-center bg-gray-900 bg-opacity-60 backdrop-blur-sm transition-opacity">
        <div class="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 overflow-hidden transform scale-100 flex flex-col max-h-[90vh]">
            ...
            <div class="bg-udinus-navy p-5 flex justify-between items-center flex-shrink-0">
                <h3 id="modal-title" class="text-xl font-bold text-white">Tambah Data Dosen</h3>
            ...
            <form action="admin-dosen.php" method="POST" enctype="multipart/form-data" class="p-6 overflow-y-auto flex-grow space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="text" name="nama_dosen" id="input-nama" required placeholder="Cth: Dr. Amiq Fahmi, S.Kom., M.Kom." class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Jabatan Akademik</label>
                        <input type="text" name="jabatan_akademik" id="input-jabatan" required placeholder="Cth: Dosen Senior / Kaprodi" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy">
                    </div>
                </div>
                
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Bidang Kepakaran</label>
                        <input type="text" name="kepakaran" id="input-kepakaran" required placeholder="Cth: Data Science & Machine Learning" class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy">
                    </div>
                    <div>
                        <label class="block text-sm font-bold text-gray-700 mb-1">Status Tampilan Data</label>
                        <select name="urutan_tampil" id="input-urutan" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy bg-white">
                            <option value="2">Dosen Reguler (Grid Biasa)</option>
                            <option value="1">Pimpinan / Kaprodi (Prioritas Atas)</option>
                        </select>
                    </div>
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">Teks Sambutan Pimpinan <span class="text-xs font-normal text-gray-400 ml-1">(Opsional, Khusus Kaprodi)</span></label>
                    <textarea name="sambutan_teks" id="input-sambutan" rows="4" placeholder="Tuliskan pesan sambutan resmi untuk ditampilkan di halaman profil..." class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:outline-none focus:ring-2 focus:ring-udinus-navy resize-none"></textarea>
                </div>

                <div class="bg-gray-50 border border-gray-200 rounded-lg p-4">
                    <label class="block text-sm font-bold text-gray-700 mb-2">Unggah Foto Resmi Dosen</label>
                    <input type="file" name="foto_dosen" id="input-gambar" accept="image/*" class="w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-udinus-navy hover:file:bg-blue-100 cursor-pointer">
                    <p id="helper-gambar" class="text-xs text-red-500 mt-2 font-medium">*Wajib melampirkan foto profesional untuk dosen baru (Maks. 3MB).</p>
                </div>
    ...
    <script>
        ...
        function bukaModal() {
            document.getElementById('modal-title').innerText = "Tambah Data Dosen";
            ...
            document.getElementById('helper-gambar').innerText = "*Wajib melampirkan foto profesional untuk dosen baru (Maks. 3MB).";
        ...
        function editData(dataLengkap) {
            ...
            // Jika isi sambutan null di DB, pastikan terkonversi jadi string kosong
            ...
            document.getElementById('helper-gambar').innerText = "*Kosongkan kolom ini jika Anda tidak ingin merubah foto dosen.";
        ...
        function konfirmasiHapus(id) {
            const setuju = confirm("Peringatan: Aksi ini tidak dapat dibatalkan.\n\nApakah Anda yakin ingin menghapus data dosen ini beserta fotonya dari server?");
            if (setuju) {
