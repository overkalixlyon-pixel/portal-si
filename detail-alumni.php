<?php
// === ALAT BANTU DEBUGGING (AKAN MENAMPILKAN ERROR JIKA ADA) ===
...
try {
    ...
    $queryAlumni->execute();
    $data_alumni_db = $queryAlumni->fetchAll(PDO::FETCH_ASSOC);

    // 5. Merakit ulang data dari Database menjadi format Array untuk disuntikkan ke JavaScript
    if ($data_alumni_db) {
        foreach ($data_alumni_db as $alumni) {

            // Normalisasi Foto Profil Dinamis (MENGGUNAKAN STRPOS AGAR AMAN DI SEMUA VERSI PHP)
            ...
            } elseif (strpos($fotoUrl, 'http') !== 0) {
                $fotoUrl = 'assets/images/' . $fotoUrl;
            }

            // Normalisasi Foto Sampul Dinamis (MENGGUNAKAN STRPOS AGAR AMAN DI SEMUA VERSI PHP)
            ...
            } elseif (strpos($coverUrl, 'http') !== 0) {
                $coverUrl = 'assets/images/' . $coverUrl;
            }

            // Penanganan Nilai Kosong Profil Aktif
            ...
            $mulaiAktif = !empty($alumni['mulai_sekarang']) ? htmlspecialchars($alumni['mulai_sekarang']) : '';
            $deskripsiAktif = !empty($alumni['deskripsi_sekarang']) ? htmlspecialchars($alumni['deskripsi_sekarang']) : '-';
            ...
            // B. Riwayat Karir / Pengalaman (Format JSON dari Database)
            $array_pengalaman = [];

            // ---------------------------------------------------------------------
            // INJEKSI PEKERJAAN SAAT INI (TAMPIL PALING ATAS PADA REKAM JEJAK)
            // ---------------------------------------------------------------------
            if (!empty($alumni['jabatan_sekarang']) && !empty($alumni['perusahaan_sekarang'])) {
                $durasi_sekarang = !empty($mulaiAktif) ? $mulaiAktif . " - Sekarang" : "Sekarang";
                $array_pengalaman[] = [
                    'title' => $jabatanAktif . " (Pekerjaan Saat Ini)",
                    'company' => $perusahaanAktif,
                    'duration' => $durasi_sekarang,
                    'desc' => $deskripsiAktif
                ];
            }

            // Menambahkan Jejak Karir Lama dari Database
            $karirRaw = trim($alumni['riwayat_karir'] ?? '');
            $karirJson = json_decode($karirRaw, true);

            if (is_array($karirJson) && !empty($karirJson)) {

                // ALGORITMA SORTING UNTUK KARIR LAMA (Dari terbaru ke terlama)
                usort($karirJson, function ($a, $b) {
                    $timeA = (empty($a['selesai']) || strtolower($a['selesai']) === 'sekarang') ? strtotime('9999-12-31') : strtotime($a['selesai']);
                    $timeB = (empty($b['selesai']) || strtolower($b['selesai']) === 'sekarang') ? strtotime('9999-12-31') : strtotime($b['selesai']);
                    if (!$timeA) $timeA = strtotime($a['mulai'] ?? '1970-01-01');
                    if (!$timeB) $timeB = strtotime($b['mulai'] ?? '1970-01-01');
                    return $timeB <=> $timeA;
                });
            ...
            // Jika kosong melompong (tidak ada saat ini & lama)
            if (empty($array_pengalaman)) {
                $array_pengalaman[] = [
                    'title' => 'Alumni Terdaftar',
                    'company' => 'Lulusan SI UDINUS',
            ...
            if (is_array($sertifJson) && !empty($sertifJson)) {
                ...
                foreach ($sertifJson as $index => $sertif) {
                    ...
                    $array_sertifikasi[] = [
                        ...
                        'url' => htmlspecialchars($sertif['url'] ?? '')
                    ];
                }
            ...
            $array_js_alumni[] = [
                'db_id' => (int) ($alumni['id'] ?? 0),
                'name' => htmlspecialchars($alumni['nama_lengkap'] ?? 'Nama Tidak Diketahui'),
    ...
    $json_data_alumni = json_encode($array_js_alumni, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
} catch (PDOException $e) {
    die("Terjadi kesalahan sistem saat memuat data profil: " . htmlspecialchars($e->getMessage()));
}
?>
<!DOCTYPE html>
...
<body class="bg-gray-50 text-gray-800 font-sans antialiased flex flex-col min-h-screen relative selection:bg-udinus-gold selection:text-white">
    ...
    <?php if (empty($array_js_alumni)): ?>
        <div class="flex-grow flex items-center justify-center min-h-screen">
            <div class="text-center bg-white p-10 rounded-3xl shadow-xl border border-gray-100 max-w-md">
                <svg class="w-20 h-20 text-gray-300 mx-auto mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                </svg>
                <h2 class="text-xl font-bold text-gray-900 mb-2">Belum Ada Data Alumni</h2>
                <p class="text-gray-500 text-sm">Direktori alumni saat ini masih kosong. Silakan kembali lagi nanti.</p>
            </div>
        </div>
    <?php else: ?>

        <!-- TOMBOL NAVIGASI SLIDER KIRI / KANAN -->
        <button onclick="slideProfile(-1)" aria-label="Profil Sebelumnya" class="fixed left-4 md:left-8 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-udinus-navy p-3 md:p-4 rounded-full shadow-xl backdrop-blur-md z-40 transition hover:scale-110 focus:outline-none border border-gray-200 flex items-center justify-center">
            <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M15 19l-7-7 7-7"></path>
            </svg>
        </button>

        <button onclick="slideProfile(1)" aria-label="Profil Selanjutnya" class="fixed right-4 md:right-8 top-1/2 transform -translate-y-1/2 bg-white/90 hover:bg-white text-udinus-navy p-3 md:p-4 rounded-full shadow-xl backdrop-blur-md z-40 transition hover:scale-110 focus:outline-none border border-gray-200 flex items-center justify-center">
            <svg class="w-5 h-5 md:w-6 md:h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <main class="flex-grow pb-24">
            ...
            <div id="profile-container" class="container mx-auto px-4 md:px-12 lg:px-20 max-w-6xl -mt-24 md:-mt-36 relative z-10 fade-transition opacity-0 translate-y-4">
        ...
        <script>
            ...
            function renderProfile(index) {
                ...
                if (profile.linkedin && profile.linkedin !== '#' && profile.linkedin.trim() !== '') {
                    linkedinContainer.innerHTML = `
                    <a href="${profile.linkedin}" target="_blank" rel="noopener noreferrer" class="w-full flex justify-center items-center gap-2 bg-[#0a66c2] hover:bg-[#004182] text-white font-bold py-3 px-4 rounded-xl transition duration-300 shadow-md shadow-[#0a66c2]/30">
                ...
                // Fade-in animation saat render pertama
                const container = document.getElementById('profile-container');
                container.style.opacity = '1';
                container.style.transform = 'translateY(0)';

                // Perbarui URL dengan ID yang sedang aktif jika tidak ada
                updateUrlParam(profile.db_id);
            }

            function updateUrlParam(id) {
                const newUrl = new URL(window.location);
                if (newUrl.searchParams.get('id') != id) {
